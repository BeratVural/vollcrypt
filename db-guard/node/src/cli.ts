...
async function migratePostgres(url: string, table: string, column: string, idCol: string, key: Buffer, version: string, chunkSize: number) {
  let Client;
  try {
  ...
  try {
    ...
    const countRes = await client.query(
      `SELECT COUNT(*) FROM "${table}" WHERE "${column}" IS NOT NULL AND "${column}" NOT LIKE 'VOLLVALT:%'`
    );
    ...
    while (true) {
      ...
      const batchRes = await client.query(
        `SELECT "${idCol}", "${column}" FROM "${table}" WHERE "${column}" IS NOT NULL AND "${column}" NOT LIKE 'VOLLVALT:%' LIMIT $1`,
        [chunkSize]
      );
      ...
      for (const row of batchRes.rows) {
        ...
        const encryptedVal = encryptValue(rawVal, key, version);
        await client.query(
          `UPDATE "${table}" SET "${column}" = $1 WHERE "${idCol}" = $2`,
          [encryptedVal, row[idCol]]
        );
        processed++;
        printProgressBar(processed, total);
      }
...
async function migrateMongo(url: string, collectionName: string, field: string, idCol: string, key: Buffer, version: string, chunkSize: number) {
  let MongoClient;
  try {
