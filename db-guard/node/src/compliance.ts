...
export function auditConfiguration(config: ComplianceAuditInput): ComplianceScorecard {
  const passed: string[] = [];
  const failed: string[] = [];
  ...
  let gdprCount = 0;
  if (hasKms) gdprCount += 25;
  if (hasRbac) gdprCount += 25;
  gdprCount += 25; // RAM Zeroization always active
  if (hasAuditLog) gdprCount += 25;
  ...
  let pciCount = 0;
  if (hasKms) pciCount += 25;
  if (hasKek) pciCount += 25;
  if (rateLimitMode === 'fail_closed') pciCount += 25;
...
export function generateComplianceHtmlReport(config: ComplianceAuditInput): string {
