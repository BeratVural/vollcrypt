...
*   **Crypto-Shredding (Purge Mode):** In addition to erasing wraps, the Node path API supports in-place `Purge` mode. It overwrites the old ciphertext body with zero bytes before truncating the file to the sealed header size. This is best-effort filesystem overwriting; storage snapshots, backups, journaling, and firmware may retain historical blocks.
