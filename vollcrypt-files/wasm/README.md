...
*   **Crypto-Shredding (Purge Mode):** In addition to erasing wraps, `Purge` mode returns a sealed copy without ciphertext. Browser/WASM byte-buffer APIs cannot erase caller-owned source buffers or storage; callers must delete or overwrite any original container bytes they persist.
