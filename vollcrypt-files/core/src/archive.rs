...
pub fn derive_file_key(dek: &[u8; 32], relative_path: &str) -> Result<[u8; 32], FileFormatError> {
    ...
    hk.expand(relative_path.as_bytes(), &mut file_key)
        .map_err(|_| {
            FileFormatError::IntegrityError("HKDF expansion failed for file key".to_string())
        })?;
    Ok(file_key)
}
...
fn collect_entries(
    ...
    for entry in fs::read_dir(dir)? {
        ...
        let rel_path = path.strip_prefix(base_dir).map_err(std::io::Error::other)?;
...
pub fn pack_directory(
    ...
    for (abs_path, rel_path, is_dir) in entries {
        ...
        if !is_dir {
            // Read file plaintext
            let plaintext = fs::read(&abs_path).map_err(|e| {
                FileFormatError::IoError(format!("Failed to read file {:?}: {}", abs_path, e))
            })?;
...
pub fn unpack_directory(
    ...
    fs::create_dir_all(dest_dir).map_err(|e| {
        FileFormatError::IoError(format!("Failed to create destination directory: {}", e))
    })?;
    ...
    for _ in 0..entry_count {
        ...
        archive_file.read_exact(&mut path_bytes).map_err(|e| {
            FileFormatError::IoError(format!("Failed to read relative path: {}", e))
        })?;
        ...
        if path_obj.is_absolute() {
            return Err(FileFormatError::IoError(format!(
        ...
        if is_dir {
            fs::create_dir_all(&target_path).map_err(|e| {
                FileFormatError::IoError(format!(
            ...
            archive_file.read_exact(&mut size_bytes).map_err(|e| {
                FileFormatError::IoError(format!("Failed to read file size: {}", e))
            })?;
            ...
            if size as u64 > remaining_bytes {
                return Err(FileFormatError::IoError(
                    "Archive file is truncated or has invalid size".to_string(),
                ));
            }
            let mut ciphertext = vec![0u8; size];
            archive_file.read_exact(&mut ciphertext).map_err(|e| {
                FileFormatError::IoError(format!("Failed to read ciphertext: {}", e))
            })?;
            ...
            fs::write(&target_path, plaintext).map_err(|e| {
                FileFormatError::IoError(format!("Failed to write file {:?}: {}", target_path, e))
            })?;
        }
    }
...
mod tests {
    ...
    fn test_pack_unpack_directory() {
        ...
        assert_eq!(
            fs::read_to_string(dest_dir.join("sub/file2.txt")).unwrap(),
