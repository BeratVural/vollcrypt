...
use crate::provider::{CryptoProvider, BoxFuture};
...
fn get_subtle_crypto() -> Result<web_sys::SubtleCrypto, FileFormatError> {
    ...
    if crypto_val.is_undefined() || crypto_val.is_null() {
        return Err(FileFormatError::IoError("crypto is undefined/null".to_string()));
    }
...
async fn check_zero_copy_support() -> bool {
    ...
    if js_sys::Reflect::set(&algorithm, &JsValue::from_str("name"), &JsValue::from_str("AES-GCM")).is_err() {
        return false;
    }
    if js_sys::Reflect::set(&algorithm, &JsValue::from_str("length"), &JsValue::from_f64(256.0)).is_err() {
        return false;
    }
    ...
    let key_promise = match subtle.import_key_with_object("raw", &key_js, &algorithm, false, &usages) {
        Ok(p) => p,
        Err(_) => return false,
    };
    ...
    if js_sys::Reflect::set(&encrypt_params, &JsValue::from_str("name"), &JsValue::from_str("AES-GCM")).is_err() {
        return false;
    }
    ...
    if js_sys::Reflect::set(&encrypt_params, &JsValue::from_str("tagLength"), &JsValue::from_f64(128.0)).is_err() {
        return false;
    }
...
impl CryptoProvider for WasmWebCryptoProvider {
    ...
    fn encrypt_async(
        ...
        Box::pin(async move {
            ...
            let algorithm = Object::new();
            js_sys::Reflect::set(&algorithm, &JsValue::from_str("name"), &JsValue::from_str("AES-GCM"))
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            js_sys::Reflect::set(&algorithm, &JsValue::from_str("length"), &JsValue::from_f64(256.0))
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            ...
            let key_val = wasm_bindgen_futures::JsFuture::from(key_promise).await
                .map_err(|e| FileFormatError::IoError(format!("importKey promise error: {:?}", e)))?;
            let crypto_key = web_sys::CryptoKey::from(key_val);
            
            // 2. Encrypt
            let encrypt_params = Object::new();
            js_sys::Reflect::set(&encrypt_params, &JsValue::from_str("name"), &JsValue::from_str("AES-GCM"))
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            ...
            js_sys::Reflect::set(&encrypt_params, &JsValue::from_str("additionalData"), &aad_js)
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            
            js_sys::Reflect::set(&encrypt_params, &JsValue::from_str("tagLength"), &JsValue::from_f64(128.0))
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            
            let enc_promise = subtle.encrypt_with_object_and_u8_array(
                &encrypt_params,
                &crypto_key,
                &plaintext,
    ...
    fn decrypt_async(
        ...
        Box::pin(async move {
            ...
            js_sys::Reflect::set(&decrypt_params, &JsValue::from_str("name"), &JsValue::from_str("AES-GCM"))
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            ...
            let aad_js = js_sys::Uint8Array::from(aad.as_slice());
            js_sys::Reflect::set(&decrypt_params, &JsValue::from_str("additionalData"), &aad_js)
                .map_err(|e| FileFormatError::IoError(format!("Reflect error: {:?}", e)))?;
            ...
            let dec_promise = subtle.decrypt_with_object_and_u8_array(
                &decrypt_params,
                &crypto_key,
                &ct_and_tag,
