...
use crate::crypt::{
    encrypt_chunk_async, decrypt_chunk_async,
    encrypt_chunk_in_place, decrypt_chunk_in_place,
};
...
fn encrypt_file_pipelined_inner<R: Read + Send + 'static, W: Write + Seek + Send + 'static>(
    ...
    if let IoWriteMode::DirectOffset = selected_mode {
        *is_direct = true;
        ...
        let source_len = if let Some(file) = try_downcast_file(source_any) {
            ...
            FileFormatError::IoError("DirectOffset mode requires a source of known size (e.g. std::fs::File)".to_string())
        })?;

        let total_chunks = if source_len == 0 { 0 } else { source_len.div_ceil(chunk_size as u64) };
        let mut total_size = header_len as u64;
        if total_chunks > 0 {
            let last_chunk_plaintext_len = (source_len % chunk_size as u64) as usize;
            let last_chunk_len = if last_chunk_plaintext_len == 0 { chunk_size } else { last_chunk_plaintext_len };
        ...
        let dest_file = try_downcast_file(dest_any).ok_or_else(|| {
            FileFormatError::IoError("DirectOffset mode requires a destination std::fs::File".to_string())
        })?;

        // Preallocate
        dest_file.set_len(total_size).map_err(|e| FileFormatError::IoError(e.to_string()))?;

        // Spawn writers
        for _ in 0..num_workers {
            let cloned_file = dest_file.try_clone().map_err(|e| FileFormatError::IoError(e.to_string()))?;
        ...
        let t = thread::spawn(move || {
            loop {
                ...
                if let Some(batch) = task {
                    ...
                    for mut t in batch {
                        let idx = t.index;
                        let res = encrypt_chunk_in_place(&dek, &file_id, idx, &mut t.buffer, t.plaintext_len);
                        match res {
                            Ok(()) => {
                                if let Some(ref file) = local_file {
                                    let offset = header_len as u64 + (idx as u64) * (32 + chunk_size as u64);
                                    if let Err(e) = crate::writer::write_raw_at(file, t.buffer.as_envelope_slice(t.plaintext_len), offset) {
...

    {
        ...
        plaintext_size = read_thread.join().unwrap().map_err(|e| FileFormatError::IoError(e.to_string()))?;
    ...
    if *is_direct {
        ...
        let dest_any: &mut dyn std::any::Any = dest;
        if let Some(file) = try_downcast_file(dest_any) {
            file.sync_all().map_err(|e| FileFormatError::IoError(e.to_string()))?;
        }
    }
...
pub fn decrypt_file_pipelined<R: Read + Send + 'static, W: Write>(
        ...
        let t = thread::spawn(move || {
            loop {
                ...
                if let Some(batch) = task {
                    ...
                    for mut t in batch {
                        let idx = t.index;
                        let res = decrypt_chunk_in_place(&dek, &file_id, idx, &mut t.buffer, t.plaintext_len);
                        match res {
                            Ok(()) => {
    ...
    while let Ok(batch_results) = write_rx.recv() {
        ...
        while let Some((buf, len)) = pending.remove(&next_expected) {
            dest.write_all(buf.as_plaintext(len)).map_err(|e| FileFormatError::IoError(e.to_string()))?;
            pool.return_buffer(buf);
            next_expected += 1;
        }
    }
...
pub async fn encrypt_file_pipelined_async(
    ...
    for batch_idx in 0..chunk_count.div_ceil(16) {
        ...
        for idx in start_chunk..end_chunk {
            ...
            futures.push(async move {
                let res = encrypt_chunk_async(&dek, &file_id, idx as u32, chunk_data).await;
                (idx, res)
            });
        }
    ...
    let serialized_header = header.write();
    let total_capacity = serialized_header.len() + encrypted_chunks.iter().map(|c| c.len()).sum::<usize>();
...
pub async fn decrypt_file_pipelined_async(
    ...
    for batch_idx in 0..(total_chunks as usize).div_ceil(16) {
        ...
        for idx in start_chunk..end_chunk {
            ...
            let file_id = file_id;
            futures.push(async move {
                let res = decrypt_chunk_async(&dek, &file_id, idx as u32, env).await;
                (idx, res)
            });
        }
