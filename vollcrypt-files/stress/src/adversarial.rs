...
mod tests {
    ...
    fn test_tag_forgery() {
        ...
        let mut env = encrypt_chunk(&dek, &file_id, 0, &plaintext).unwrap();
        ...
        for _ in 0..attempts {
            ...
            if decrypt_chunk(&dek, &file_id, 0, &env).is_ok() {
                successful_forgeries += 1;
            }
        }

        assert_eq!(successful_forgeries, 0, "A random tag was accepted as valid!");
        println!("SECURITY AUDIT: Tag forgery resistance verified (0 / {} random tags accepted).", attempts);
    }
    ...
    fn test_header_tampering_matrix() {
        ...
        serialized[0] ^= 0xFF;
        assert!(Header::parse(&serialized).is_err(), "Header parsing should fail with tampered magic");
        serialized[0] ^= 0xFF; // Restore

        // 2. Tamper Version byte
        serialized[8] = 99;
        assert!(Header::parse(&serialized).is_err(), "Header parsing should fail with unsupported version");
        ...
        serialized[39] ^= 0xFF;
        let (parsed, _) = Header::parse(&serialized).unwrap();
        assert_ne!(parsed.merkle_root, [9u8; 32], "Merkle root tampering undetected");
    ...
    fn test_replay_and_substitution() {
        ...
        let res = decrypt_chunk(&dek, &file_id_b, 0, &env_a);
        assert!(res.is_err(), "Cross-file chunk substitution was not rejected!");

        // Index substitution
        let res_idx = decrypt_chunk(&dek, &file_id_a, 1, &env_a);
        assert!(res_idx.is_err(), "Chunk index substitution was not rejected!");
    }
    ...
    fn test_manifest_authority_tampering() {
        ...
        let (unauthorized_pk, unauthorized_sk) = ed25519_keypair_generate();
        ...
        let mut forged_signed_op = SignedOperation {
            ...
            prev_hash,
            timestamp: 1234567,
            signer_pubkey: unauthorized_pk,
            data_len: data.len() as u32,
            data,
            signature: [0u8; 64],
        };
        let msg = forged_signed_op.sig_message();
        forged_signed_op.signature = ed25519_sign(&unauthorized_sk, &msg);
        ...
        assert!(manifest.verify().is_err(), "Tampered manifest authority was verified successfully!");
    }
    ...
    fn test_signed_header_tampering() {
        ...
        let sig = ed25519_sign(&signer_sk, &msg);
        ...
        assert!(verify_header_signature_plain(&tampered_header).is_err(), "Signature verification succeeded on tampered header!");
    }
}
