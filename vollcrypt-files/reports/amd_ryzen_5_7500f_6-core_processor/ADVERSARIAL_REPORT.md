...
| **K.2 shield_timing_constant** | Merkle root comparisons in ShieldPolicy verification use constant-time operations. | *REJECTED* | REJECTED: Constant-time subtle::ConstantTimeEq comparison verified | **✓ Defended** |
...
## Section J — Sovereign Sealing & Crypto-Shredding

### J.1 sovereign_sealed_fail_closed (RESOLVED)
Once a container is sealed, its wrap table is cleared, making it mathematically impossible to recover the DEK. Standard decryption routes reject the container immediately with `ContainerSealed` error.

### J.2 sealed_marker_tamper (RESOLVED)
For signed containers, the sealed marker is signed by the owner. Modifying or stripping the signature or marker is detected by the integrity checks.

## Section K — Shield Integrity Policy

### K.1 shield_verified_fail_closed (RESOLVED)
Under the strict default Shield policy (Verified Release Mode), a complete double-pass verification is performed before releasing any plaintext. Any tampering triggers a fail-closed behavior, resulting in exactly 0 bytes released.

### K.2 shield_timing_constant (RESOLVED)
All Merkle root cryptographic comparisons are executed using constant-time equality checks (`subtle::ConstantTimeEq`) to prevent side-channel timing analysis.
