...
exports.DbProxyServer = exports.ClusterManager = exports.FipsStartupError = void 0;
...
class ClusterManager {
    ...
    verifyMessage(msg) {
        ...
        const now = Date.now();
        if (Math.abs(now - msg.timestamp) > 5000)
            return false;
        const unsignedMsg = {
            type: msg.type,
            senderId: msg.senderId,
        ...
        try {
            const expectedBuf = buffer_1.Buffer.from(this.signMessage(unsignedMsg), 'hex');
            ...
            for (const [key, expiresAt] of this.acceptedSignatures) {
                if (expiresAt <= now)
                    this.acceptedSignatures.delete(key);
            }
            const replayKey = msg.senderId + ':' + msg.signature;
            if (this.acceptedSignatures.has(replayKey))
                return false;
...
class DbProxyServer {
    ...
    async start() {
        if (this.options.fipsMode) {
            ...
            this.logSiemEvent('FIPS_INIT', 1, 'system', '127.0.0.1', 'Node.js/OpenSSL FIPS mode is active. Deployment validation remains the operator responsibility.');
        }
        if (this.options.xorKeySplitShares !== undefined) {
            if (this.options.allowSingleProcessXorKeySplit !== true) {
                throw new Error('Single-process XOR key split is n-of-n compatibility behavior, not MPC; explicit acknowledgement is required');
            }
            const reconstructedKey = (0, mpc_js_1.reconstructKeyFromXorShares)(this.options.xorKeySplitShares, this.options.xorKeySplitExpectedShares);
        ...
        if (clusteringConfigured) {
            ...
            if (processLocalControls.length > 0) {
                throw new Error(`Cluster mode rejects process-local security controls: ${processLocalControls.join(', ')}. Configure a distributed authorization and rate-limit service`);
            }
        }
    ...
    createPostgresConnectionRuntime(clientSocket) {
        ...
        return {
            clientSocket,
            backendSocket,
            clientParser: new pg_protocol_js_1.PostgresStreamParser(),
            backendParser: new pg_protocol_js_1.PostgresStreamParser(),
            userContext: null,
            originalUsername: '',
            dbGuardContext: {
                role: 'GUEST',
    ...
    attachPostgresClientListeners(runtime, socket) {
            ...
            try {
                ...
                for (const msg of messages) {
                    ...
                    if (!runtime.userContext) {
                        ...
                        runtime.userContext = (0, auth_js_1.resolveUserContext)(username, this.options.config);
                        ...
                        if (type === 81) { // 'Q' (Simple Query)
                            ...
                            runtime.currentAggregates = (0, waf_js_1.identifyAggregates)(queryStr);
                            runtime.rowCount = 0;
                            ...
                            try {
                                ...
                                if (activeJit) {
                                    if (activeJit.expiresAt > Date.now()) {
                                        ...
                                        const originalContext = (0, auth_js_1.resolveUserContext)(runtime.originalUsername, this.options.config);
                                ...
                                if (!this.options.noWaf) {
                                    (0, waf_js_1.validateQuery)(queryStr, runtime.dbGuardContext.role);
                                ...
                                const rewritten = (0, waf_js_1.rewriteQuery)(queryStr, runtime.dbGuardContext.role, runtime.userContext?.tenantId, this.options.config);
    ...
    attachPostgresBackendListeners(runtime) {
            ...
            try {
                ...
                for (const msg of messages) {
                    ...
                    if (type === 84) { // 'T' -> RowDescription
                        runtime.currentColumns = (0, pg_protocol_js_1.parseRowDescription)(msg);
                        ...
                        for (let i = 0; i < values.length; i++) {
                            ...
                            if (strVal.startsWith('VOLLVALT:')) {
                                ...
                                try {
                                    ...
                                    const decrypted = db_guard_1.dbGuardContextStore.run(runtime.dbGuardContext, () => {
