...
function scanSql(query: string): SqlScanResult {
  ...
  let state: 'normal' | 'single' | 'double' | 'backtick' | 'bracket' | 'line' | 'block' | 'dollar' = 'normal';
  let blockDepth = 0;
  let dollarTag = '';
  ...
  for (let i = 0; i < query.length; i++) {
    ...
    else if (char === String.fromCharCode(96)) state = 'backtick';
...
function containsEncodedSqlControl(query: string): boolean {
  if (/%(?:25)*(?:00|22|26|27|2d|2f|3b|3d|5c)/i.test(query)) return true;
  if (/\\u(?:0000|0022|0026|0027|002d|002f|003b|003d|005c)/i.test(query)) return true;
