...
fn key_log_modify_timestamp_breaks_chain() {
    ...
    assert!(
        result.is_err(),
        "Modified timestamp must break the chain's validity"
    );
...
fn key_log_duplicate_entry() {
    ...
    assert!(
        result.is_err(),
        "Cannot append duplicate entry (prev_hash mismatch contextually)"
    );
}
...
fn key_log_current_key_nonexistent_user() {
    let log = KeyLog::new();
    assert!(
        log.current_key_for(b"bob").is_none(),
        "Must not panic on unknown user"
    );
}
...
fn key_log_key_at_timestamp_before_first_entry() {
    ...
    assert!(
        log.key_at_timestamp(b"alice", 500).is_none(),
        "Returns None if before first entry"
    );
}
...
fn key_log_revoke_nonexistent_key() {
    ...
    assert!(
        result.is_err(),
        "Cannot verify revoke if there is no previous valid key"
    );
}
